...
class Simulation:
    ...
    def get_init_note(self):
        print(list(self.tpm.keys()))
        #return [55]
        #return [random.choice(list(self.tpm.keys()))]
        if self.track.voice == 'soprano':
            return [random.choice([60, 84])]
        elif self.track.voice == 'alto':
            return [random.choice([53, 77])]
        elif self.track.voice == 'tenor':
            return [random.choice([48, 72])]
        elif self.track.voice == 'bass':
            print([random.choice([40, 64])])
            return [random.choice([40, 64])]
    ...
    def next_state(self, initial_note, l):
        if l > 0:
            ...
            if duration < .8:
                ...
                d = 1000
            self.track.track.append(self.to_midi_message(next[0], d))
            l -= 1
